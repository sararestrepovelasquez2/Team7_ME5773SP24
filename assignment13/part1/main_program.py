...
#                 x 
#
# The  second quad is formed by the nodes 1, 2, 5 and 4. 
# 
...
# center point location, point (0,2).
x = points[:,0]-0
y = points[:,1]-2
grid['dcenter'] = (x**2 + y**2)**0.5
...
v = np.zeros((points.shape[0],points.shape[1]), dtype=np.float64 )
v[:,0] = points[:,1]
v[:,1] = -points[:,0]
grid[ 'velocity' ] = v
